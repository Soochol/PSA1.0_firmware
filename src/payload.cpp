...
void usartMessage::initialize() {
    reset();  // Simple initialization - just reset the buffer
}

// Note: add_gaussian_noise function implementation removed
// If noise generation is needed in the future, implement it properly with required includes


usartMessage::usartMessage(void) {
    buffer = new(std::nothrow) char[USART_MESSAGE_MAXIMUM_LENGTH];
    if (!buffer) {
        // Handle allocation failure gracefully
        bufferIndex = 0;
        return;
...
void usartMessage::appendByte(byte value) {
    if (!buffer) return;  // Safety check for failed allocation
    
    if (bufferIndex < USART_MESSAGE_MAXIMUM_LENGTH - 1) {  // -1 for safety margin
        buffer[bufferIndex] = static_cast<char>(value); 
        bufferIndex++; // Move the index to the next available position
    }
    // Note: Silent overflow protection - buffer remains in valid state
}