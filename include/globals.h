/**
 * @file globals.h
 * @brief Global definitions, configurations, and data structures for PSA1.0 Firmware
 * 
 * Developer Notes:
 * (cshin)
...
// STM32 hardware communication is used by default:
// - ESP_LOG = UART0 (USB) → Debug output
// - STMSerial = UART2 (GPIO 15/16) → STM32 hardware communication
#define LTESerial Serial1               // UART1 → LTE modem
#define STMSerial Serial2               // UART2 (GPIO 15/16) → STM32 hardware communication
...
typedef struct {
    ...
    uint32_t stepCount = 0;             // cumulative values
    uint32_t lmaCount = 0;
...
class biosigDataClass {
    public:
        ...
        uint16_t addData(allData_t allData);    // adds new data to buffers including torsoAngle & does tilt calculation
